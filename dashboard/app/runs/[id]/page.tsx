              ...
              <table style={{ width: "auto" }}>
                ...
                <tbody>
                  {gs.map(([k, ts]) => {
                    ...
                    return (
                      <tr key={k}>
                        <td className="taskcol">{k}{(ts[0].task?.data?.answer ?? ts[0].task?.data?.secret ?? ts[0].task?.data?.name) != null && <span className="muted"> → {String(ts[0].task.data.answer ?? ts[0].task.data.secret ?? ts[0].task.data.name)}</span>}</td>
                        {ts.map((t) => (
